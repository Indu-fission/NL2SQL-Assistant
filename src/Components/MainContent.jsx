...
const MainContent = ({ query, setQuery, onSubmit, onClear, isSidebarOpen, setQueryHistory, autoRun, setAutoRun }) => {
  ...
  return (
    <div className="bg-white p-6 rounded-md shadow-md text-gray-800 border border-gray-200">
      <h1 className="text-3xl font-semibold mb-1" style={{ color: '#b58932' }} > {t('title')}</h1>
      <p className="text-sm text-gray-600 mb-4">
        {t('subtitle')}
      </p>
      ...
      <div className="flex items-center gap-4">
        <button
          onClick={handleProcess}
          disabled={isLoading}
          className={`px-4 py-2 rounded-md text-white flex items-center gap-2 transition ${isLoading ? 'bg-orange-300' : 'bg-orange-500 hover:bg-orange-600'
            }`}
        >
          {isLoading && <FiLoader className="animate-spin" />}
          {/* {isLoading ? 'Processing...' : 'Process Query'} */}
      ...
      {loadedTabs.length > 0 && (
        <div className="mt-8">
          <h2 className="text-xl font-semibold mb-4" style={{ color: '#b58932' }}>{t('pipelineTitle')}</h2>
          {totalExecutionTime && (
            <p className="text-sm text-gray-600 mb-4">
          ...
          <div className="relative border-b border-gray-300 flex space-x-6 pb-2">
            {loadedTabs.map((tab) => (
              <button
                ...
                onClick={() => setActiveTab(tab.name)}
                disabled={!tab.executionTime}
                className={`text-sm font-medium pb-2 transition border-b-2 ${activeTab === tab.name
                    ? 'border-orange-500 text-orange-600'
                    : 'text-gray-600 hover:text-orange-500 border-transparent hover:border-orange-300'
                  } ${!tab.executionTime ? 'text-gray-400 cursor-not-allowed' : ''}`}
              >
