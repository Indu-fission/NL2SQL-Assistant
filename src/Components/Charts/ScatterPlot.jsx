...
const ScatterPlot = ({ data, xKey, yKey }) => {
  ...
  // ... (Your existing validation checks for xKey, yKey, data remain the same)
  if (!xKey || !yKey || xKey === yKey) {
    return (
      <div className="flex flex-col items-center justify-center h-[480px] text-gray-500 font-sans p-4 text-center">
        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor" className="w-16 h-16 mb-3 opacity-40">
  ...
  if (plotData.length === 0) {
    return (
      <div className="flex flex-col items-center justify-center h-[480px] text-gray-500 font-sans p-4 text-center">
        ...
        <p className="text-gray-600 font-medium text-sm">Scatter Plot requires at least 2 numerical columns to display</p>
      </div>
    );
  }
  ...
  // ... (End of your existing validation checks)
  ...
  return (
    <div className="w-full font-sans">
      ...
      <div>
        <ResponsiveContainer width="100%" height={520}>
          <ScatterChart
            // MODIFIED: More generous margins to prevent label overlapping
            margin={{ top: 20, right: 150, bottom: 185, left: 115 }}
            onMouseMove={(e) => {
              if (e && e.activePayload && e.activePayload.length > 0) {
                setHoveredPoint(e.activePayload[0].payload.__index__);
              } else { setHoveredPoint(null); }
          ...
          >
            ...
            <XAxis
              ...
              // MODIFIED: Increased dy for XAxis ticks for better spacing with angled text
              tick={{ fill: '#000', fontSize: 11, fontWeight: 700, angle: -45, textAnchor: 'end', dy: 10 }}
              domain={[0, uniqueXValues.length > 0 ? uniqueXValues.length - 1 : 0]}
              interval={uniqueXValues.length > 10 ? Math.floor(uniqueXValues.length / Math.min(8, uniqueXValues.length > 0 ? uniqueXValues.length : 1 )) : 0}
              // MODIFIED: Increased height for XAxis to accommodate potentially longer angled labels
              height={120}
            >
              {/* MODIFIED: Increased offset for XAxis Label to push it further down */}
              <Label value={capitalize(xKey)} position='outsideBottom' offset={60} style={{ textAnchor: 'middle', fill: '#000', fontSize: '13px', fontWeight: '800' }} />
            </XAxis>
            <YAxis
              type="number"
              dataKey="__y__"
              ...
              tick={{ fill: '#000', fontSize: 11, fontWeight: 700, dx: -5 }}
              tickFormatter={yAxisTickFormatter}
              // MODIFIED: Adjusted YAxis width
              width={70}
            >
              {/* MODIFIED: Increased offset (more negative) for YAxis Label to push it further left */}
              <Label value={yAxisLabelValue} angle={-90} position='outsideLeft' offset={-75} style={{ textAnchor: 'middle', fill: '#334155', fontSize: '13px', fontWeight: '600' }} />
            </YAxis>
            <Tooltip content={<CustomTooltip xKey={xKey} yKey={yKey} xLabelToColorMap={xValueToColorMap} />} cursor={{ stroke: '#94a3b8', strokeWidth: 1, strokeDasharray: '3 3' }} wrapperStyle={{ zIndex: 1000 }} />
            <Legend 
              layout="vertical" 
